...
function populateConfig(lambdaPath, callback) {
    ...
    switch (projectConfig.naming) {
        ...
        case 'snake':
            ...
            break;
        default:
            callback('Unkown naming scheme: %s', projectConfig.naming);
    }
...
function buildJS(lambdaName, callback) {
}
...
function buildNodeModules(lambdaName, callback) {
    ...
    async.each(modules,
        function(moduleName, done) {
            ...
            fs.writeFile(buildPkgJSONPath, JSON.stringify(pkgJSON), function(err) {
                if (err) {
